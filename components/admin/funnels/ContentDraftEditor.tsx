...
import { GripVertical, Trash2, Plus, ChevronDown, ChevronRight, Copy } from 'lucide-react'
...
export function ContentDraftEditor({ value, onChange }: { value: CopyDraft; onChange: (v: CopyDraft) => void }) {
  ...
  const addCustomBlock = () => {
    setBlocks([...blocks, { kind: 'custom', content: { intent: '', markdown: '' } }])
  }

  if (!blocks.length) {
    return (
      <div className="text-center py-8 text-neutral-500 text-sm border border-dashed border-neutral-800 rounded-lg">
        Chưa có block nào. Bấm "Draft nội dung với AI" ở trên để AI generate copy_draft.
      </div>
    )
  }
  ...
  return (
    <div className="space-y-2">
      ...
      <button
        ...
        className="w-full py-2 border border-dashed border-neutral-700 rounded-lg text-sm text-neutral-500 hover:text-white hover:border-neutral-500"
      >
        <Plus className="w-4 h-4 inline mr-1" /> Thêm custom block
      </button>
    </div>
  )
}

function BlockCard({ block, index, total, expanded, onToggle, onRemove, onMove, onUpdate }: {
  block: Block; index: number; total: number; expanded: boolean;
  onToggle: () => void; onRemove: () => void; onMove: (dir: -1 | 1) => void; onUpdate: (patch: any) => void
}) {
    ...
    <div className="border border-neutral-800 rounded-lg bg-neutral-900/30">
      <div className="flex items-center gap-2 p-2 border-b border-neutral-800/50">
        ...
        <button onClick={onToggle} className="flex-1 flex items-center gap-2 text-left">
          ...
          <span className="text-xs px-1.5 py-0.5 bg-neutral-800 rounded font-mono">{block.kind}</span>
